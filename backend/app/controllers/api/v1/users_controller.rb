  ...
  module V1
    class UsersController < BaseController
      ...
      def check_favorite
        ...
        is_favorite = current_user.favorites.exists?(manga_id: manga_id)
        render json: { is_favorite: is_favorite }
      end
      
      def toggle_favorite
        manga_id = params[:manga_id]
        favorite = current_user.favorites.find_by(manga_id: manga_id)
        
        if favorite
          favorite.destroy
          render json: { is_favorite: false, message: "Đã xóa khỏi danh sách yêu thích" }
        else
          favorite = current_user.favorites.create(manga_id: manga_id)
