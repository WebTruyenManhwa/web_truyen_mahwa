...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/"); // Chuyển hướng đến trang chủ sau khi đăng ký và đăng nhập
    } catch (err: unknown) {
      setIsLoading(false);
      if (err && typeof err === 'object' && 'response' in err) {
        const errorResponse = err.response as { data?: { message?: string } };
        setError(errorResponse?.data?.message || "Đăng ký thất bại. Vui lòng thử lại sau.");
      } else {
  ...
  const handleGoogleRegister = () => {
    setError("");
    // Chuyển hướng đến API endpoint xác thực Google
  ...
  const handleFacebookRegister = () => {
    setError("");
    alert("Chức năng đăng ký bằng Facebook đang được phát triển. Vui lòng sử dụng đăng ký bằng email và mật khẩu.");
  };
    ...
    <div className="max-w-md mx-auto">
      <div className="bg-gray-800 rounded-lg p-8 shadow-lg">
        ...
        <div className="mt-8 pt-6 border-t border-gray-700">
          <p className="text-center text-gray-500 text-sm mb-4">Hoặc đăng ký với</p>
